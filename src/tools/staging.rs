...
pub async fn git_commit(ctx: ToolContext, input: GitCommitInput) -> Result<GitCommitOutput> {
    ...
    let commit_hash = output.stdout
        .lines()
        .find(|l| l.contains('[') && l.contains(']'))
        .and_then(|l| {
            let start = l.find('[')?;
            let end = l.find(']')?;
            Some(l[start+1..end].split_whitespace().next()?.to_string())
        });
    
    // Get current branch
    let branch_output = executor.execute(&["branch", "--show-current"])?;
...
pub async fn git_diff(ctx: ToolContext, input: GitDiffInput) -> Result<GitDiffOutput> {
    ...
    let (files_changed, insertions, deletions) = if input.stat.unwrap_or(false) {
        ...
        let fc = parts.get(0).and_then(|s| s.split_whitespace().next()).and_then(|s| s.parse().ok());
        let ins = parts.get(1).and_then(|s| s.split_whitespace().next()).and_then(|s| s.parse().ok());
        let del = parts.get(2).and_then(|s| s.split_whitespace().next()).and_then(|s| s.parse().ok());
        (fc, ins, del)
    } else {
        (None, None, None)
    };
